...
public class JoinKStreamKTableIT extends KafkaIntegrationTest {
  @Test
  void endToEndJoin() {
    System.setProperty("stream.topic", "stream-join-it");
    System.setProperty("table.topic", "table-join-it");
    System.setProperty("output.topic", "joined-it");

    Properties props = new Properties();
    props.put(StreamsConfig.APPLICATION_ID_CONFIG, "join-kstream-ktable-it");
    ...
    try (KafkaProducer<String, String> producer = new KafkaProducer<>(prodProps)) {
      producer.send(new ProducerRecord<>("table-join-it", "k1", "T1"));
      producer.send(new ProducerRecord<>("stream-join-it", "k1", "S1"));
      producer.send(new ProducerRecord<>("stream-join-it", "k2", "S2"));
      producer.flush();
    }
    ...
    try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(consProps)) {
      ...
      assertThat(records.iterator().next().value()).isEqualTo("S1:T1");
    }
